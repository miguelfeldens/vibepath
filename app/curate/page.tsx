...
const PHASE_LABELS: Record<string, string> = {
  ...
  branch:  'Lookbook — Moodboard set 1 (people & purpose)',
  branch2: 'Lookbook — Moodboard set 2 (success & aspiration)',
  branch3: 'Lookbook — Moodboard set 3 (place & aesthetic)',
}
...
type SourcedCandidates = { unsplash: QuizImage[]; pexels: QuizImage[] }
type CandidateMap = Record<string, SourcedCandidates>
type SelectionMap = Record<string, QuizImage[]>

const PER_SOURCE = 5

async function fetchCandidates(phase: string, impact: Impact): Promise<{ unsplash: QuizImage[]; pexels: QuizImage[] }> {
  const config = PHASE_CONFIGS[phase][impact]
  const unsplash: QuizImage[] = []
  const pexels: QuizImage[] = []

  // Unsplash — always fetch PER_SOURCE images
  if (UNSPLASH_KEY) {
    try {
      const res = await fetch(
        `https://api.unsplash.com/search/photos?query=${encodeURIComponent(config.query)}&per_page=${PER_SOURCE}&orientation=landscape`,
        { headers: { Authorization: `Client-ID ${UNSPLASH_KEY}` } }
      )
      if (res.ok) {
        const data = await res.json()
        for (const p of (data.results ?? []).slice(0, PER_SOURCE)) {
          unsplash.push({
            id: p.id,
            url: p.urls.regular,
  ...
  // Pexels — always fetch PER_SOURCE images independently (not a fallback)
  if (PEXELS_KEY && PEXELS_KEY !== 'your_pexels_api_key_here') {
    try {
      const res = await fetch(
        `https://api.pexels.com/v1/search?query=${encodeURIComponent(config.query)}&per_page=${PER_SOURCE}&orientation=landscape`,
        { headers: { Authorization: PEXELS_KEY } }
      )
      if (res.ok) {
        const data = await res.json()
        for (const p of (data.photos ?? []).slice(0, PER_SOURCE)) {
          pexels.push({
            id: `pexels_${p.id}`,
...
export default function CuratePage() {
  ...
  const totalPools = PHASES.length * IMPACTS.length // 50
    ...
    const load = async () => {
      for (const phase of PHASES) {
        for (const impact of IMPACTS) {
          ...
          const sourced = await fetchCandidates(phase, impact)
          setCandidates(prev => ({ ...prev, [key]: sourced }))
        ...
        {PHASES.map(phase => (
          <div key={phase}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
              {IMPACTS.map(impact => {
                const key = poolKey(phase, impact)
                const sourced = candidates[key]
                const selected = selections[key] ?? []
                const isLoaded = key in candidates

                const renderImages = (images: QuizImage[], sourceLabel: string, accentClass: string) => (
                  <div>
                    <p className={`text-[10px] font-semibold uppercase tracking-widest mb-1.5 ${accentClass}`}>{sourceLabel}</p>
                ...
                return (
                  <div key={key}>
                    <p className="text-xs text-white/40 font-medium tracking-wide uppercase mb-3">
                    ...
                    {!isLoaded ? (
                      ...
                      <div className="space-y-4">
                        {renderImages(sourced?.unsplash ?? [], 'Unsplash', 'text-sky-400/70')}
                        {renderImages(sourced?.pexels ?? [], 'Pexels', 'text-amber-400/70')}
                      </div>
                    )}
                  </div>
                )
